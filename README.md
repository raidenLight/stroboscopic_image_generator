...
| 帧选取 | K标记、R范围、清除、间隔选取 |
| Alpha | 首/末帧渐变、背景alpha、叠序切换、彩色蒙版 |
...
- **后帧在上 / 前帧在上**（Alpha 行按钮）：控制多帧物体的叠加顺序。默认"后帧在上"（新帧覆盖旧帧），切换后旧帧覆盖新帧。
- **逐物体蒙版着色**：`Ctrl + 点击` 帧列表的物体列头，可单独开关每个物体的半透明彩色蒙版。列头显示 🎨 表示该物体已着色。便于在多物体合成图中区分各物体轨迹。
